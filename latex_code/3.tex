...
$$
    \hat{Y}_i = \bh_0 + \cdots + \bh_k x_{ik} = \x_i^T\Bh.
$$
...
$$
    \Bh 
    = \argmin_{\B\in\R^{k+1}} \sum_{i=1}^n (Y_i-\x_i^T\B)^2
    = \argmin_{\B\in\R^{k+1}} (\Y-\X\B)^T(\Y-\X\B).
$$
or we may use the \term{maximum likelihood estimator} (MLE). It turns out that with our assumptions the result is the same. Differentiating and equating to zero yields:
$$
    \Bh = \underbrace{(\X^T\X)^{-1}\X^T}_{\mH}\Y.
$$
...
Our next goal is to estimate $\s^2$. The MLE is given by $\Sh^2 = \frac{1}{n}\Eh^T\Eh$, but this is skewed as $\ev{\Eh^T\Eh}=\s^2(n-p)$. Hence our unbiased estimator is:
$$
    \Sh^2 = \frac{1}{n-p}(\Y-\X\B)^T(\Y-\X\B).
$$
...
\begin{figure}[H]\centering
    \begin{tikzpicture}
        ...
        \addplot [red, domain=-4:-2.01, fill=red!30] {1/(sqrt(2*pi))*exp(-x^2/2)} \closedcycle;
        \addplot [red, domain=2.01:4, fill=red!30] {1/(sqrt(2*pi))*exp(-x^2/2)} \closedcycle;
        \draw[red, dashed] (-2.01,0) -- (-2.01,0.39);
        \draw[red, dashed] (2.01,0) -- (2.01,0.39);
        \draw[<->] (-2.5,0.1) -- node[above] {\(\alpha/2\)} (-0.5,0.1);
        \draw[<->] (0.5,0.1) -- node[above] {\(\alpha/2\)} (2.5,0.1);
        \draw[<->] (-2.5,0.45) -- node[above] {\(1-\alpha\)} (2.5,0.45);
        \draw[<->] (-2.5,0) -- node[below] {\(-t_{\alpha/2, n-1}\)} (-0.5,0);
        \draw[<->] (0.5,0) -- node[below] {\(t_{\alpha/2, n-1}\)} (2.5,0);
        \end{axis}
    \end{tikzpicture}    
    \caption{\TODO{fix figure}}
\end{figure}

We have seen that $\Bh\sim N(\B, \s^2(\X^T\X)^{-1})$. Denote $(\X^T\X)^{-1}=(e_{ij})_{i,j=1,\dots,p}$. We then have $\bh_j\sim N(\b_j, \s^2 e_{jj})$, so $\frac{\bh_j-\b_j}{\sqrt{e_{jj}}\s}\sim N(0,1)$. Since the variance is unknown, consider the statistic:
$$
...
\TODO{questions about independence. Detour into sigma algebras etc ...}
...
\begin{proof}
    ...
    $$
        ...
        =
        \underbrace{\sum_{i=1}^{n} \overbrace{(Y_i-\hat{Y}_i)}^{\e_i}\hat{Y}_{i}}_{=\Eh^T\hat{\Y}=0}
...
$$
As usual, we assume $\bs{\delta}=(\delta_1,\dots,\delta_n)^T\sim N(0, \s^2\mI)$. We may then estimate $\bs{\alpha}=(\alpha_0,\dots,\alpha_{k-1})^T$ and $\s^2$ by the usual $\hat{\bs{\alpha}}, \sh^2$ to obtain fitted $\hat{x}_{ik}$. We find the squared empirical correlation between $x_{i,k}$ and $\hat{x_{i,k}}$ as:
$$
    R_k^2 
...
\begin{align*}
    \mr{SST} = \sum_{i=1}^n(Y_i-\bar{Y})^2 = (\mC\Y)^T(\mC\Y) = \Y^T \mC \Y = \Y^T\brac{\mI-\frac{1}{n}\one\one^T}\Y.
\end{align*}
For the residual sum of squares we also need the fact that $\mH\x_i=\x_i$ for all columns of $\X$. This follows readily as $\mH\X=\X(\X^T\X)^{-1}\X^T\X=\X$. From this we have $\mH\one = \one$ as this is the first column of $\X$. 
\begin{align*}
    \mr{SSR} 
    &= \sum_{i=1}^n(\hat{Y}_i-\bar{Y})^2 
    = (\mC\mH\Y)^T(\mC\mH\Y) 
    = \Y^T\mH\mC\mH\Y 
    \\&= \Y^T(\mH-\frac{1}{n}\mH\one\one^T\mH^T)\Y 
    = \Y^T(\mH-\frac{1}{n}\one\one^T)\Y    
\end{align*}
About the matrix $\mH-\frac{1}{n}\one\one^T$, we note that it is symmetric, idempotent and of rank $p-1$:
$$
    \rank{\mH-\frac{1}{n}\one\one^T} = \tr{\mH-\frac{1}{n}\one\one^T} = \tr{\mH}-\frac{1}{n}\tr{\one\one^T}=p-1.
$$
Finally, for the error sum of squares we obtain using that $\mI-\mH$ is symmetric and idempotent:
$$
    \mr{SSE} = \sum_{i=1}^n(\hat{Y}_i-Y_i)^2 = \Eh^T\Eh = ((\mI-\mH)\Y)^T(\mI-\mH)\Y = \dots = \Y^T(\mI-\mH)\Y.
$$
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{F-test}
